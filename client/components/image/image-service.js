angular.module('plantzrApp').service('ImageService', function ($q, $timeout) {

    var NUM_LOBES = 3
    var lanczos = lanczosGenerator(NUM_LOBES)

    // resize via lanczos-sinc convolution
    this.resize = function (imgSrc, factor) {

        if (!(factor > 0 && factor < 1)) {
            throw new Error('Scale down must be between 0 and 1');
        }
        ...
        var width = Math.floor(img.naturalWidth * factor);
        var height = Math.floor(img.naturalHeight * factor);
    ...
    this.resizeStep = function (imgSrc, width, height, quality) {
